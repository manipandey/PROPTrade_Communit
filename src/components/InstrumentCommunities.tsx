...
export default function InstrumentCommunities({ currentUser, onOpenAuth }: InstrumentCommunitiesProps) {
  ...
  useEffect(() => {
    ...
    const savedVotes = localStorage.getItem('alphajournal_inst_votes');
    const savedUserVote = localStorage.getItem('alphajournal_inst_user_votes');
    ...
    if (savedVotes) {
      ...
      setVotes(initial);
      localStorage.setItem('alphajournal_inst_votes', JSON.stringify(initial));
    }
  ...
  const handleVote = (type: 'bullish' | 'bearish') => {
    ...
    setVotes(prev => {
      ...
      localStorage.setItem('alphajournal_inst_votes', JSON.stringify(updated));
      return updated;
    });
    ...
    setUserVote(prev => {
      ...
      localStorage.setItem('alphajournal_inst_user_votes', JSON.stringify(updated));
      return updated;
    });
  };
  ...
  return (
    <div className="relative min-h-[500px]">
      ...
      {!currentUser?.loggedIn && (
        <div className="absolute inset-0 z-35 flex items-start justify-center p-4 pt-28" style={{ background: 'rgba(0,0,0,0.15)' }}>
          <div className="w-full max-w-md rounded-2xl border border-border-theme bg-bg-card/95 p-8 text-center space-y-6 glass-panel shadow-[0_12px_40px_rgba(0,0,0,0.5)] animate-fade-in pointer-events-auto">
            ...
            <div className="space-y-2">
              <h3 className="text-lg font-bold text-text-primary uppercase tracking-wider font-sans">Instrument Groups Locked</h3>
      ...
      <div className={`mx-auto max-w-6xl space-y-6 transition-all duration-300 ${!currentUser?.loggedIn ? 'blur-[4px] pointer-events-none select-none' : ''}`}>
